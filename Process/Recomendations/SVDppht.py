...
spark = SparkSession.builder.appName("SVDPPht").master(spark_cluster_path).getOrCreate()
...
start =time.time()
# Set up the ALS (Alternating Least Squares) matrix factorization model for implicit feedback
als = ALS(userCol="userId", itemCol="productId", ratingCol="rating", implicitPrefs=True)
...
best_model = ALS(rank=best_rank, maxIter=best_max_iter, regParam=best_reg_param,
                 userCol="userId", itemCol="productId", ratingCol="rating", implicitPrefs=True).fit(ratings)
...
stop = time.time()
time_taken_minutes = (stop - start) / 60
